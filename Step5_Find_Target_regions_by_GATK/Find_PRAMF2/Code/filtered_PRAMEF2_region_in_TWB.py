...
#SBATCH -A MST109178
#SBATCH -J filtered_PRAMEF2_region_in_{sample_name}
#SBATCH -p ngs92G
#SBATCH -c 14
#SBATCH --mem=92g
#SBATCH -o {input_dir}/{sample_name}.out.log
#SBATCH -e {input_dir}/{sample_name}.err.log
...
--java-options "-Xmx92g" GenotypeGVCFs \
-R /staging/biology/elephant123/GATK/hg38/hg38.fa \
-V {input_dir}/{sample_name}.g.vcf.gz \
-O {output_dir}/{sample_name}.vcf.gz
rm {input_dir}/{sample_name}.g.vcf.gz
rm {input_dir}/{sample_name}.g.vcf.gz.tbi
mkdir {output_dir}/{sample_name}/PRAMEF2
#Select the region around the PRAMEF2 gene, extending 10 KB upstream and downstream
/work/opt/ohpc/Taiwania3/pkg/biology/GATK/gatk_v4.2.3.0/gatk \
--java-options "-Xmx92g" SelectVariants  \
-R /staging/biology/elephant123/GATK/hg38/hg38.fa  \
-L chr1:12847086-12871909 \
-V {output_dir}/{sample_name}.vcf.gz \
-O {output_dir}/{sample_name}/PRAMEF2/{sample_name}_in_PRAMEF2.vcf.gz
/work/opt/ohpc/Taiwania3/pkg/biology/GATK/gatk_v4.2.3.0/gatk \
--java-options "-Xmx7G -XX:ParallelGCThreads=1" VariantFiltration \
-R /staging/biology/elephant123/GATK/DRAGEN_hg38/hg38.fa \
-V {output_dir}/{sample_name}/PRAMEF2/{sample_name}_in_PRAMEF2.vcf.gz \
-O {output_dir}/{sample_name}/PRAMEF2/{sample_name}_hard_filter_in_PRAMEF2.vcf.gz \
...
-buildver hg38 \
-out {output_dir}/{sample_name}_in_PRAMEF2C \
-vcfinput  {output_dir}/{sample_name}/PRAMEF2/{sample_name}_hard_filter_in_PRAMEF2.vcf.gz \
...
mv {output_dir}/{sample_name}.vcf.gz.tbi  {output_dir}/{sample_name}
mv {output_dir}/{sample_name}.vcf.gz {output_dir}/{sample_name}
...
'''
    ...
    script_path = os.path.join(input_dir, "{}_slurm_script.sh".format(sample_name))
...
# Example usage
input_dir = "/staging/biology/elephant123/269_BC_without_BRCA_mutant"#Dictionary of tmporarily put all GVCF File
